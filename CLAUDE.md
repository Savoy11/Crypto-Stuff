...
| Coins (Coin Registry) | `/assets` | 🟢 Live | Nav label "Coins"; route path kept as `/assets` to preserve deep links. Market-breadth KPIs, asset-type chips + inline screener, sortable/paginated table (Stock-Registry-standard layout), Reserve Monitor tab. **No Safety Score / Risk Band columns and no score screener** — removed 2026-08-18 (item 4); a sortable score column over the universe is a leaderboard. Per-coin risk explanation lives on the detail page — and since 2026-08-29 (RP-6) **only inside the Composite Risk panel**: the header Safety Score gauge, the identity band pill and the search-result score were removed as bare ratings, while the panel gained rendered pillar descriptions, inline evidence and a "How this number is calculated" disclosure (scale, composition, missing-data renormalisation, coverage vs confidence, overrides, and what the score does **not** assess). Guarded by `lib/risk/__tests__/methodologyDisclosure.test.ts`. Live prices; metadata from static `assetCatalog.ts` (reference data, not mock) |
