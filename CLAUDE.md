...
| Retirement Planner | ~~`/retirement`~~ | ⚪ Removed | **Module removed 2026-08-20** with Budget (same decision). `lib/retirement/` and its 53 tests deleted; recoverable from git history (`git log --diff-filter=D`) if the separate tool wants the engine — the §402(g) shared-cap and monthly-compounding fixes live there |
