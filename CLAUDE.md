...
| Reserves | `/reserves` | 🟢 Live | DefiLlama stablecoin supply + collateralization (`/live-data/reserves`). **Linked in the crypto nav since 2026-07-29** — it previously had no nav entry and no inbound link from anywhere in `src/`, so a live page was reachable only by typing the URL. ⚠ It **duplicates** the "Reserve Monitor" tab inside `/assets`: same route, same attestation/composition sections, same `Vault` icon. The standalone page additionally carries `SourceLine` + `ProvenanceNotice` (the `stablecoinMeta` staleness disclosure) that the tab lacks — which is a gap in the *tab*, not a reason to keep both. Still wants a keep-one decision |
