...
| Staking Discovery | `/staking-discovery` | 🟢 Live | `/live-data/staking-discovery` — live on-chain pools (DefiLlama/Yearn/Pendle/Beefy) plus the curated platform directory. **Per-pool Safety badge removed 2026-08-18** (item 4 follow-up): the owner kept the page's discovery function and cut its suggestion component — a safety badge beside an APY column reads as a verdict on which pool to pick. The route still returns `riskCanonical`/`band`; nothing renders them |
...
| Crypto Scanner | `/scanner` | 🟢 Derived | Items 6/7 (2026-08-19): one scanner per section, promoted to nav. Seven setup detectors, three timeframes, bounded-concurrency scan, auto-refresh (its source is keyless). A scanner sweeps a universe to find candidates; the TA page charts the one you picked |
...
| Wallets | `/wallets` | 🟢 Live | On-chain balances for watched addresses + connected browser wallets (`/live-data/wallet/*`), plus the Pump Report tab. **DB-backed** since 2026-08-18 (NT3) via `/api/user/wallets` (+`/[id]` PATCH/DELETE) — optimistic store, client-UUID ids, one-time `fn:wallets` localStorage import that merges additively. **Exchange API linking removed 2026-08-18** on security grounds — see the secret-stores note above |
...
| Budget | `/budget`, `/budget/transactions`, `/budget/manage` | 🟢 User data | BUDGET module (ROADMAP Phase 2): accounts (balance = opening anchor + transactions), manual entry, idempotent CSV import (import-hash dedupe; saved per-bank column mappings auto-matched by header signature), rule-based auto-categorization (first-match-wins, server-side), monthly budgets vs actuals (unbudgeted ≠ $0), recurring detection (suggestions until confirmed). **Management UI shipped 2026-08-19 (NT1)** at `/budget/manage` (nav: Budget → Settings): categorization-rule manager (create/prioritize/enable/delete, optional apply-to-existing), category editor (two-level tree), account rename/archive — the schema's soft-delete path was previously unreachable — recurring confirm/**dismiss**/pause/delete, and import-profile rename/delete. Two API gaps were filled for it: `import-profiles/[id]` (the one budget table without full CRUD) and a stored `dismissed` flag on `recurring_rules` (migration 0003), since detection re-runs each load and "no thanks" had nowhere to live. Pure logic in `lib/budget/` (vitest), persistence via `/api/user/budget/*`. No external providers — no SourceLine on these pages |
| Retirement Planner | `/retirement` | 🟢 Derived | RETIREMENT module. Accumulation projection across six vehicles (pre-tax/Roth 401k, Roth/traditional IRA, brokerage, crypto), each with its own deferral %, balance and assumed return; employer match; emergency-fund runway; straight-line drawdown to a plan-to age; loan amortization and credit-card payoff. Ported from the owner's planning spreadsheet — see `lib/retirement/` for the two places the arithmetic deliberately differs from it (monthly not annual compounding; caps enforced not merely displayed). **Monthly expenses can be filled from Budget actuals** (`UseBudgetSpend`, NT1 extension) — a 3-complete-month average, disclosing its uncategorized share; it fills once rather than staying in sync, so a plan stays reproducible. **§402(g) and IRA caps are applied to the SHARED group**, not per account. Pure engine, 53 vitest tests. No external providers — no SourceLine; IRS limits carry a `ProvenanceNotice` |
...
| Strategy Backtests | `/equities/backtests` | 🟢 Derived | `security-ohlcv` real history; SMA/RSI/MACD vs buy-and-hold; per-side fee tiers (0/5/10/25 bps) with the fee math in the tested `lib/utils/equityBacktest.ts` |
