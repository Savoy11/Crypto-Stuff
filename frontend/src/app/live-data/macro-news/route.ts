...
// parsePubDate/classifyPillar are unit-tested pure helpers and live in a plain
// module: a Next.js route.ts may only export route fields, so exporting them
// here would break `next build`. The type is re-exported below for consumers
// that still import it from this route.
import { parsePubDate, classifyPillar, type MacroPillar } from '@/lib/server/macroNews'
