...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_reserve_attestations_asset_date",
        "reserve_attestations",
    ...
    op.create_index(
        "ix_liquidity_metrics_asset_timestamp", "liquidity_metrics", ["asset_id", "timestamp"]
    )
    ...
    op.create_index(
        "ix_blockchain_metrics_asset_timestamp", "blockchain_metrics", ["asset_id", "timestamp"]
    )
    op.execute(
        "SELECT create_hypertable('blockchain_metrics', 'timestamp', if_not_exists => TRUE);"
    )
    ...
    op.create_index(
        "ix_wallet_concentration_asset_timestamp", "wallet_concentration", ["asset_id", "timestamp"]
    )
    op.execute(
        "SELECT create_hypertable('wallet_concentration', 'timestamp', if_not_exists => TRUE);"
    )
